...
double PAOA_Circuit(const std::vector<double>& beta, const Eigen::MatrixXd& J_cost,const Eigen::VectorXi& rank,  int num_layers,int num_sweeps_layer, std::mt19937& gen) {
    
    // Initialize the state vector 'm' with random spins (-1 or +1)
    Eigen::VectorXd m = ((Eigen::VectorXd::Random(J_cost.rows()).array() > 0).cast<double>() * 2.0 - 1.0).matrix();
    ...
    double cost = -0.5 * m.dot(J_cost * m);
    return cost;
}



double run_experiments(const std::vector<double>& beta, const Eigen::MatrixXd& J_cost,const Eigen::VectorXi& rank,  int num_layers, int num_experiments, int num_sweeps_layer) {
    ...
    #pragma omp parallel 
    {
        ...
        for (int i = 0; i < num_experiments; i++) {
            double cost = PAOA_Circuit(beta, J_cost,rank,  num_layers,num_sweeps_layer, gen);
            global_sum += cost;
        }
    }
